...
// Whether App Check has cleared.
// Firestore holds one document tree per signed in user, so the settings account section stays hidden until this is true.
object AppCheckState {
    ...
    // Staged: a stale cache needs a forced refresh, a provider that never installed needs installing again.
    ...
    // appCheckProviderFactory() has one copy per source set: firebase-appcheck-debug is debugImplementation only.
